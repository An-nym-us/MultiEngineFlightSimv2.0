...
#Edited and altered by Jonthan Gunderson
...
from stable_baselines import DQN, PPO2, A2C, ACKTR, ACER, SAC, TD3, TRPO
...
class UnrealEnvWrap(gym.Env):
 ...
 #sending actions to UE and recieving observations in response to those actions  
  def step(self, action):
    ...
    print(UEdone)
    if done == True:
        print("Im restarting now how fun")
        reward = 0
...
def recieve(sid, data):
    ...
    # Load the trained agent
        ...
        if (algselected == 'DQN') and (usecustomparams == 'true'):
            ...
            print("Firing H.E.R. system")
            
            
            verboseval = herparams["verbose"]
            policy = herparams["policy"]
        ...
        #model_class = DQN  # works also with SAC, DDPG and TD3
            ...
            action_noise = OrnsteinUhlenbeckActionNoise(mean=np.zeros(n_actions), sigma=float(0.5) * np.ones(n_actions))
             ...
             # Wrap the model
            model = HER(policy, env, model_class, n_sampled_goal=4, goal_selection_strategy=goal_selection_strategy, verboseval=verboseval)
            
            
            print("HER training in process...")
            # Train the model
            model.learn(total_timesteps=trainepisodes)
            
            print("HER training complete")
            ...
            act_funcval = trpoparams["act func"]
            timesteps_per_batchval = trpoparams["timesteps per batch"]
            verboseval = trpoparams["verbose"]
            tensorboard_logval = trpoparams["tensorboard log"]
            _init_setup_modelval = trpoparams["init setup model"]
            full_tensorboard_logval = trpoparams["full tensorboard log"]
            seedval = trpoparams["seed"]
            n_cpu_tf_sessval = trpoparams["n cpu tf sess"]
            layersval = trpoparams["layers"]                   
            max_klval = trpoparams["max kl"]   
            cg_itersval = trpoparams["cg iters"]
            
            lamval = trpoparams["lam"]  
            entcoeffval = trpoparams["entcoeff"]
            
            cg_dampingval = trpoparams["cg damping"]

            #vf_stepsizeval = trpoparams["vf stepsize"]
            vf_stepsizeval = .0003
            vf_itersval = trpoparams["vf iters"]
